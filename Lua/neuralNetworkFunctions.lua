...
SessionFileName = "proofOfConcept_Pool.txt"
PreviousGenerationFileName = "proofOfConcept_ArchivePool"
ButtonNames = {
		"P1 A",
		"P1 B",
		"P1 Down",
...
-- For proof of concept purposes, these values are hard-coded.
-- In later versions, these values will be derived through a simple formula.
InputXDimension = 11
InputYDimension = 11
InputSize = InputXDimension * InputYDimension

InputCount = InputSize+1
...
function convertInputTableToArray(inputTable)
	local inputArray = {}
	
	-- Set input nodes
	for iy = 1, InputYDimension do
		for ix = 1, InputXDimension do
			inputArray[#inputArray + 1] = inputTable[ix][iy]
		end
	end
...
function getOutputs(genome, inputTable)
	local inputArray = convertInputTableToArray(inputTable)
